...
\textbf{S\'{a}mi languages} are a group of nine low-resource languages that belong to the family of Uralic languages. They are spoken in the S\'{a}pmi region, across Norway, Sweden, Finland, and Russia. 
North S\'{a}mi, the most widely spoken, has approximately $25{,}000$ speakers, yet it remains classified as endangered by UNESCO.

\textbf{Challanges} in Optical Character Recognition (OCR) for North S\'{a}mi range from scarce training data to complex orthographic features, making it a difficult task to streamline the digitalization efforts. Additionally, the language characteristics of S\'{a}mi leads to a large number of inflected word forms, further complicating accurate recognition. While state-of-the-art models like TrOCR achieve high accuracy (\trocrBestCacc\%), their computational demands ($\sim$\trocrSize, $\sim$\trocrBestTime\,s/img inference on CPU) make them ill-fitted for deployment in resource-constrained environments.
...
(National Library of Norway), $\sim$307k line images (276k training + 31k hyperparameter tuning), 32$\times$800px.  \textbf{Evaluation} data used \benchmarkSamples\ samples: \hfTestSamples\ in-domain synthetic lines (Spr\r{a}kbanken validation split) + \bookTestSamples\ parallel Northern S\'{a}mi--English sentences from Turi's 1910 "An Account of S\'{a}mi".
...
\textbf{Training.} AdamW (lr $10^{-4}$), batch 32, 100 epochs with
early stopping, CTC loss, trained on a single NVIDIA V100. Inference
benchmark on AMD Ryzen 5 PRO CPU.

\textbf{Negative result.} Pretrained VGG-16/19 and ResNet-50 backbones
did not converge.

\textbf{Metrics Used.} \textbf{OCR:} Character Error Rate (CER), Word Error Rate (WER), Character/Word Accuracy = $100-$CER/WER. \textbf{MT:} BLEU, chrF, TER on \bookTestSamples\ parallel pairs. \textbf{Efficiency:} model size (MB), CPU inference time (s/img).
}


\block{Accuracy vs. Sentence Length}{
\centering
\includegraphics[width=0.95\linewidth]{figures/model_comparison_sentence_length.pdf}\\[2mm]
\footnotesize Character accuracy vs.\ sentence length.\\
The character accuracy drops for longer sentences lines due to the fixed
32$\times$800\,px input.\\[3mm]
}
...
\textbf{CNN-CTC} achieves \ctcCER\% CER (\ctcCacc\% char. acc.). 


% ---- Recognition example with images --------------------------------------
\begin{center}
\textit{Example predictions - ground truth, and error types.} %\vspace{2mm}
...
\small{\textbf{Prediction:} \textit{oddasis eal\'{a}skahttojuvvon) Duohkama h\'{a}lddahusla\v{s}}} \\
\midrule
...
% \begin{tabular}{c}
% \includegraphics[width=0.92\linewidth]{figures/correct2.jpg} \\[1mm]
% \footnotesize GT: \textit{mij\'{a}v juohkkal\'{a}hk\'{a}j kreatijvvan.} \\
% \footnotesize Pred: \textit{mij\'{a}v juohkkal\'{a}hk\'{a}j kreatijvvan.} \quad \textbf{CER\,=\,0\%} \\[4mm]
% \includegraphics[width=0.92\linewidth]{figures/diacritical_errors.jpg} \\[1mm]
% \footnotesize GT: \textit{o\dj\dj asis eal\'{a}skahttojuvvon) Suohkana h\'{a}lddahusla\v{s}} \\
% \footnotesize Pred: \textit{oddasis eal\'{a}skahttojuvvon) Duohkama h\'{a}lddahusla\v{s}} \quad \textbf{CER\,=\,8.3\%} \\
% \end{tabular}
% \end{center}
% \footnotesize Diacritic stripping (\dj$\rightarrow$d) and case-confusable shapes
% account for most residual errors.\\[4mm]

}

\block{Architecture}{
\centering
\includegraphics[width=0.93\linewidth]{figures/architecture_tikz.pdf}
...
\block{4.2 Results: Other Findings}{

% ---- Domain shift table ---------------------------------------------------
\textbf{Generalization: in vs. out of domain.}\\[2mm]
\begin{center}
\small
\begin{tabular}{@{}lrrr@{}}
\toprule
Model & \shortstack{HF CER\\(\%)} & \shortstack{Book CER\\(\%)} & \shortstack{$\Delta$\\(pp)} \\
\midrule
...
\end{center}
\vspace{1mm}
The best TrOCR degrades by \trocrSynthDeltaCER\,pp on out-of-domain historical
text, while CNN-CTC is stable (\ctcCER\% $\to$ \ctcBookCER\%).

% ---- Conjunction-splitting figure -----------------------------------------
\textbf{Conjunction-based splitting on long sentences.}\\[1mm]
\begin{center}
\includegraphics[width=0.95\linewidth]{figures/split_comparison.pdf}
...
\textbf{End-to-end OCR$\to$MT.} On \bookTestSamples\ book sentences:
...
\begin{center}
  ...
  \includegraphics[width=0.41\linewidth]{figures/github-qr-code.png}
\end{center}
% \end{minipage}
}
